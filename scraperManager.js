...
const LOG_LEVEL = 3
; // Default to warnings and errors only
...
export class ScraperManager {
  constructor() {
    ...
    skyboxSync.initializeSkyboxClient({
      ...
      applicationToken:
        process.env.SKYBOX_APP_TOKEN || "44ba3709-9cb4-4d81-a7c0-e1b5247fa60b",
    });
  }
    ...
    try {
      await session.withTransaction(async () => {
        ...
        if (validScrapeResult?.length > 0) {
          // Check if this is the first scrape for the event
          const isFirstScrape = (event.metadata?.iterationNumber || 0) === 0;
          ...
          const currentGroups = await ConsecutiveGroup.find(
            { eventId },
            {
              ...
              "inventory.mapping_id": 1,
            }
          )
            .lean()
            .session(session);

          // If this is the first scrape, try to initialize event data from Skybox
          if (isFirstScrape && event.mapping_id) {
            try {
              this.logWithTime(
                `Initializing event ${eventId} data from Skybox for first scrape`,
                "info"
              );

              const skyboxInit = await skyboxSync.initializeEventFromSkybox(
                eventId,
                event.mapping_id,
                {
                  logLevel: LOG_LEVEL,
                  accountId:
                    event.skybox_account_id || process.env.SKYBOX_ACCOUNT_ID,
                }
              );

              if (skyboxInit.success && skyboxInit.inventories.length > 0) {
                this.logWithTime(
                  `Successfully initialized ${skyboxInit.inventories.length} inventories from Skybox for event ${eventId}`,
                  "success"
                );

                // Map Skybox inventories to groups
                for (const group of validScrapeResult) {
                  // Find matching inventory in Skybox data
                  const matchingInventory = skyboxInit.inventories.find(
                    (inv) =>
                      inv.section === group.section && inv.row === group.row
                  );

                  if (matchingInventory) {
                    // Update group with Skybox data
                    group.inventory = {
                      ...group.inventory,
                      mapping_id: matchingInventory.mapping_id.toString(),
                      inventoryId: matchingInventory.inventoryId.toString(),
                      listPrice: matchingInventory.listPrice,
                      notes: matchingInventory.notes,
                      publicNotes: matchingInventory.publicNotes,
                      stockType: matchingInventory.stockType,
                      splitType: matchingInventory.splitType,
                      customSplit: matchingInventory.customSplit,
                      hideSeatNumbers: matchingInventory.hideSeatNumbers,
                    };

                    if (LOG_LEVEL >= 2) {
                      this.logWithTime(
                        `Mapped Skybox inventory ${matchingInventory.inventoryId} to group ${group.section} ${group.row}`,
                        "info"
                      );
                    }
                  }
                ...
                // Store the Skybox event data in the event document
                await Event.updateOne(
                  { Event_ID: eventId },
                  {
                    $set: {
                      skybox_event_data: skyboxInit.eventData,
                      skybox_mapping_complete: true,
                    },
                  }
                ).session(session);
              }
              ...
              this.logWithTime(
                `Error initializing event data from Skybox: ${error.message}`,
                "error"
              );

              // Log to error database but continue with regular flow
              await this.logError(eventId, "SKYBOX_INIT_ERROR", error, {
                message: "Failed to initialize event data from Skybox",
              });
            }
          }
          ...
          if (hasChanges) {
            ...
            // Prepare the validScrapeResult with mapping_ids from the database
            const enrichedScrapeResult = validScrapeResult.map((group) => {
              // Look for existing mapping_id from database for the same section/row within the same event
              // Only consider groups that have seating information
              const existingGroup = currentGroups.find(
                (cg) =>
                  cg.section === group.section &&
                  cg.row === group.row &&
                  cg.seats &&
                  cg.seats.length > 0 &&
                  cg.inventory?.mapping_id
              );

              // Create a copy of the group with the mapping_id included
              return {
                ...group,
                inventory: {
                  ...group.inventory,
                  // Use mapping_id from the same event or fallback options
                  mapping_id:
                    existingGroup?.inventory?.mapping_id ||
                    group.mapping_id ||
                    group.skybox ||
                    "",
                },
            ...
            try {
              ...
              if (syncResult.success) {
                ...
                // Update the enrichedScrapeResult with any new mapping IDs from Skybox
                if (syncResult.changes.added.length > 0) {
                  for (const addedItem of syncResult.changes.added) {
                    const matchingGroup = enrichedScrapeResult.find(
                      (group) =>
                        group.section === addedItem.section &&
                        group.row === addedItem.row
                    );

                    if (matchingGroup) {
                      matchingGroup.inventory = {
                        ...matchingGroup.inventory,
                        mapping_id: addedItem.inventoryId.toString(),
                      };

                      if (LOG_LEVEL >= 2) {
                        this.logWithTime(
                          `Updated mapping ID for ${matchingGroup.section} ${matchingGroup.row} to ${addedItem.inventoryId}`,
                          "info"
                        );
                      }
                    }
            ...
            const groupsToInsert = validScrapeResult.map((group) => {
              // Find corresponding enriched group with possibly updated mapping_id
              const enrichedGroup = enrichedScrapeResult.find(
                (eg) => eg.section === group.section && eg.row === group.row
              );
              ...
              return {
                ...
                inventory: {
                  ...
                  mapping_id:
                    enrichedGroup?.inventory?.mapping_id ||
                    group.inventory.mapping_id ||
                    group.mapping_id ||
                    group.skybox ||
                    "",
  ...
  async generateInventoryCsv(eventId, scrapeResult) {
    try {
      ...
      if (!inventoryController.isFirstScrape(eventId)) {
        if (LOG_LEVEL >= 2) {
          this.logWithTime(
            `Skipping CSV generation for event ${eventId} - not the first scrape`,
            "info"
          );
        }
        return;
      ...
      if (LOG_LEVEL >= 1) {
        this.logWithTime(
          `Generating CSV for event ${eventId} (first scrape)`,
          "info"
        );
      }
      ...
      // Check for existing groups to get mapping_ids
      const existingGroups = await ConsecutiveGroup.find({ eventId })
        .select("section row inventory.mapping_id")
        .lean();
      ...
      for (const group of validGroups) {
        ...
        // Format in-hand date as YYYY-MM-DD
        const formattedInHandDate =
          inHandDate instanceof Date
            ? inHandDate.toISOString().split("T")[0]
            : new Date().toISOString().split("T")[0];

        // Find existing group with matching section/row to get mapping_id
        // Only consider groups that have seating information in the same event
        const existingGroup = existingGroups.find(
          (eg) =>
            eg.section === group.section &&
            eg.row === group.row &&
            eg.seats &&
            eg.seats.length > 0 &&
            eg.inventory?.mapping_id
        );

        // Create a record with the exact required format
        inventoryRecords.push({
          inventory_id: `${eventId}-${group.section}-${group.row}-${Math.min(
            ...group.seats
          )}`,
          ...
          barcodes: group.seats
            .map(
              () =>
                `${Math.floor(Math.random() * 1000000000000)}${Math.floor(
