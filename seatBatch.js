...
export function GetMapSeats(data) {
...
export function breakArray(arr) {
  // Exact same implementation as before
  let result = [];
  let subarray = [arr[0]];
...
export function getSplitType(arr, offer) {
  // Exact same implementation as before
...
export function CreateInventoryAndLine(data, offer, event, descriptions) {
  // Exact same implementation as before
  ...
  if(isNameAdded==false)
  {
    if(_descriptions)
    {
      _descriptions.descriptions.map(x=>{
        ...
        else if(x.toLowerCase().includes("deaf") || x.toLowerCase().includes("blind"))
        {
          allDescriptions+=", deaf/hard, blind/low"
        }
      })
  ...
  let singleExtraCharges=parseFloat(parseFloat(offer?.charges.filter(x=>x?.reason=="order_processing").reduce((total, item) => total + item.amount, 0))/data?.seats.length);
  let repeatExtraCharges=parseFloat(offer?.charges.filter(x=>x?.reason!="order_processing").reduce((total, item) => total + item.amount, 0));
...
export function AttachRowSection(data, mapData, offers, event, descriptions) {
  // Exact same implementation as before
