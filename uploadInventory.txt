...
import { ENABLE_CSV_UPLOAD } from './scraperManager.js';
...
async function uploadInventoryCycle() {
  try {
    // Check if CSV uploads are enabled
    if (!ENABLE_CSV_UPLOAD) {
      console.log('CSV uploads are disabled (ENABLE_CSV_UPLOAD=false). Skipping upload cycle.');
      return;
    }
...
async function startContinuousUpload() {
  try {
    // Check if CSV uploads are enabled
    if (!ENABLE_CSV_UPLOAD) {
      console.log('CSV uploads are disabled (ENABLE_CSV_UPLOAD=false in scraperManager.js).');
      console.log('To enable uploads, set ENABLE_CSV_UPLOAD=true in scraperManager.js and restart.');
      return;
    }
