...
function CreateInventoryAndLine(data,offer,event,descriptions)
{
 ...
 if (Array.isArray(data.attributes) && data.attributes.includes("obstructed")) {
  ...
  // Check for hearing/sight accessibility (these we keep but add description)
  if(data?.accessibility) {
    const accessibilityArray = Array.isArray(data.accessibility) ? data.accessibility : [data.accessibility];
    const accessibilityString = accessibilityArray.join(' ').toLowerCase();
    
    if(accessibilityString.includes("sight") || accessibilityString.includes("hearing") || 
       accessibilityString.includes("deaf") || accessibilityString.includes("blind")) {
  ...
  if (_descriptions?.descriptions) {
    _descriptions.descriptions.forEach(x => {
      ...
      // Skip row descriptions (e.g., "2 row", "3 row", etc.)
      if (desc.match(/^\d+\s+row$/)) {
        return; // Don't add row descriptions to public notes
      }
...
export const AttachRowSection = (data, mapData, offers, event,descriptions) => {
 ...
 let allAvailableSeats = GetMapSeats(mapData);
  ...
  // Filter out accessibility seats early in the process
  const filteredData = data.filter(x => {
    if (x.accessibility) {
      const accessibilityArray = Array.isArray(x.accessibility) ? x.accessibility : [x.accessibility];
      const accessibilityString = accessibilityArray.join(' ').toLowerCase();
      
      // Check for specific accessibility keywords that should be excluded
      ...
      // Only exclude if it contains these specific keywords
      return !excludeKeywords.some(keyword => accessibilityString.includes(keyword));
    }
    return true; // Include seats without accessibility info
  });
  
  let mapPlacesIndex = allAvailableSeats.map(x => x.seatId);
  let returnData=[];
  //get all seats number by seat id
  let customData=filteredData.map(x => {
 ...
 // Filter out accessible seats based on keywords
 returnData = returnData.filter(item => {
  if (item.accessibility) {
    const accessibilityString = String(item.accessibility).toLowerCase();
    // Keywords derived from the image: 
    // Companion Seat, Deaf/Hard of hearing/Blind/Low vision, Semi-ambulatory Seat, Wheelchair Accessible
    const accessibilityKeywords = [
      "companion", 
      "deaf", "hard of hearing", "blind", "low vision", "sight", "hearing", // for Deaf/Hard of hearing/Blind/Low vision
      "semi-ambulatory", "ambulatory", // for Semi-ambulatory Seat
      "wheelchair" // for Wheelchair Accessible
    ];
    for (const keyword of accessibilityKeywords) {
      if (accessibilityString.includes(keyword)) {
        return false; // Exclude this item if any accessibility keyword is found
      }
    }
  }
  return true; // Include this item if no accessibility keywords are found or if accessibility is not defined
});
