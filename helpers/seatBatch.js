...
/**
 * Breaks down map data into individual seat objects
 * @param {Object} data - The map data to process
...
function GetMapSeats(data) {
  ...
  fs.writeFileSync("seatsArray.json", JSON.stringify(seatArray, null, 2));
...
function CreateInventoryAndLine(data, offer, event, descriptions) {
  ...
  if (
    ...
    data.attributes.includes("obstructed")
  ) {
  ...
  */
  ...
  // Format all prices to 2 decimal places to prevent validation errors
  const formattedTotalCost = parseFloat(totalCost.toFixed(2));
  const formattedFaceValue = parseFloat(faceValue.toFixed(2));
  const formattedTotalCostWithPercentage = parseFloat(
    totalCostWithPercentage.toFixed(2)
  );
  ...
  return {
    inventory: {
      ...
      row: data?.row || "",
      cost: formattedTotalCost * (data?.seats?.length || 0),
      seats: data?.seats || [],
      ...
      listPrice: formattedTotalCostWithPercentage,
      ...
      tickets: data?.seats.map((y) => {
        return {
          ...
          faceValue: formattedFaceValue,
          taxedCost: formattedTotalCost,
          sellPrice: formattedTotalCostWithPercentage,
